...
public class Company {
	...
	// Default constructor will have no data 
	public Company() {
		System.out.println("Default - Empty Contructor");
	}
	...
	public double profit(Company comp, double capital) {
		...
		try {
			...
			// Show a Message Dialog as an error message
			if (comp.area == "" || comp.area == " " || (comp.area.isBlank()) || comp.area == null)
				JOptionPane.showMessageDialog(null, "Input Error! Area cannot be empty!", "Area Input Problem",
	...
	public double logistic(double distance, boolean membership) {
		...
		try {
			...
			// cannot be negative number. Show a message dialog as as error message.
			if (distance < 0 || distance > 12743)
				JOptionPane.showMessageDialog(null, "Input Error! Given value is out of boundaries!", 
						"Distance Input Problem", JOptionPane.ERROR_MESSAGE);
	...
	public boolean nameCheck(Company comp1, Company comp2) {
		try {
			...
			if (comp1.name.isBlank() || comp2.name.isBlank()) {
				JOptionPane.showMessageDialog(null, "Input Error! One of the companies' name is missing!", 
						"Name Input Problem", JOptionPane.ERROR_MESSAGE);
			}
	...
	public String CEOofComp(Company comp, String name, String surName, int age, int exp) {
		...
		try {
			...
			if (age < 15 || age > 87) 
				JOptionPane.showMessageDialog(null, "Youngest CEO in the World is 15. Oldest CEO in the World is 87.", 
						"Unrealistic Age Input", JOptionPane.ERROR_MESSAGE);
			...
			try {
				...
				if (answer == 'Y' || answer == 'y' || answer == 'n' || answer == 'N') {
					...
					JOptionPane.showMessageDialog(null, "You miss the boat! Goodbye.", 
							"WHAT THE F#%K? YOU'RE A DUMB.", JOptionPane.ERROR_MESSAGE);
				}
